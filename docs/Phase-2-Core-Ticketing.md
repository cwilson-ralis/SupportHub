...
Also create `ITicketMessageService` in `Core/Interfaces/` so reply/message behavior is explicit:

```csharp
public interface ITicketMessageService
{
 Task<Result<TicketMessageDto>> AddReplyAsync(int ticketId, AddReplyDto dto);
 Task<Result<List<TicketMessageDto>>> GetMessagesAsync(int ticketId);
}
```
...
2. Create DTOs in `Core/DTOs/`:
 ...
 - **`TicketMessageDto`**: Id, TicketId, Body, Direction, SenderEmail, SenderName, ReplySenderType?, ExternalMessageId, CreatedAt, Attachments (list)
 - **`AddReplyDto`**: Body (required), SendAsEmail (bool), ReplySenderType? (v1 supports SharedMailbox only when `SendAsEmail = true`), AttachmentIds (List\<int\>?)
 ...
 **Business rules to enforce:**
 ...
 - `New` -> `Open`, `AwaitingAgent`, `Closed`
 - `Open` -> `AwaitingCustomer`, `AwaitingAgent`, `OnHold`, `Resolved`, `Closed`
 - `AwaitingCustomer` -> `AwaitingAgent`, `OnHold`, `Resolved`, `Closed`
 - `AwaitingAgent` -> `AwaitingCustomer`, `OnHold`, `Resolved`, `Closed`
 - `OnHold` -> `Open`, `AwaitingAgent`, `AwaitingCustomer`, `Resolved`, `Closed`
 - `Resolved` -> `Open`, `Closed`
 - `Closed` -> `Open`
 - Any transition outside the matrix returns a business validation error and does not change state.
...
1. **Layout:**
 - Top bar: Company selector dropdown (filtered to user's assigned companies, SuperAdmin sees all), Queue (Division) dropdown (filter by routing queue), "New Ticket" button
 ...
 **Left column (70% width) - Conversation:**
 ...
 - Sender indicator: show "From: [shared mailbox]"; personal mailbox sender mode is future scope
...
`POST /api/v1/tickets/{id}/messages` should be backed by `ITicketMessageService.AddReplyAsync` with `AddReplyDto`.
...
1. Update the Blazor `MainLayout` to include a sidebar with:
 ...
 - Knowledge Base (placeholder, grayed out until Phase 6)
 - Admin section (visible to Admin/SuperAdmin):
 - Companies
 - Users
 - Queues (Divisions) (placeholder, grayed out until Phase 4)
