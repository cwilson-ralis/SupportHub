...
- Support ticketing (Phase 1)
- Knowledge base and self-service (Phase 2+)
...
| **TicketMessage** | Body, sender, direction (inbound/outbound), reply-from preference (shared mailbox vs. agent), timestamps. Supports rich text and embedded images. |
...
- AI suggestions are applied as a pre-routing step before the rules engine, or as a fallback when rules yield no match.
...
- Reply to existing thread: matched via `X-Ticket-Id` header (injected into all outbound replies) or subject line token. Appended as a new `TicketMessage` on the existing `Ticket`.
- AI classification step invoked if routing rules return no match.

### Outbound

When an agent replies, the system sends via Graph API. Agent can select the send-from address:
- Shared mailbox (default — ticket reply appears from the company support address)
- Agent's personal mailbox (for direct escalations)

The outbound message is recorded as a `TicketMessage` (direction: Outbound). The `X-Ticket-Id` header is always injected to maintain thread matching on future replies.
...
- When a threshold is approaching or breached, an `SLABreach` record is created and a notification surfaces in the UI (and optionally via email).
...
- Email-to-ticket thread matching (`X-Ticket-Id` header + subject fallback)
...
- **⚠️ Graph API permissions** — Mail.ReadWrite admin consent required for shared mailboxes. IT must be involved early. This is a hard dependency for Phase 3.
- **Email threading reliability** — A ticket ID token injected into the `X-Ticket-Id` header is more reliable than subject line matching alone. Subject matching is the fallback only.
...
- **⚠️ Timeline** — Pong's target is 2 weeks for Phase 1. The team flagged that requirements gathering and planning alone will take several days. A solid foundation with proper architecture will move faster in subsequent phases. Recommend confirming Phase 1 scope and sprint plan in the next working session.
