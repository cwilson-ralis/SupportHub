...
1. **Azure AD App Permissions** - Document in `docs/GraphApiSetup.md`:
 ...
 - `Mail.Send` (application) - send mail from configured shared mailboxes
...
{
 Task<Result<string>> SendReplyAsync(int ticketId, int messageId);
}
```
 ...
 **Flow for `SendReplyAsync`:**
 ...
 - v1: always send from Company.SharedMailboxAddress
 3. Build the email:
 ...
 - POST /users/{sharedMailboxAddress}/sendMail
 ...
 **Future scope (post-v1):**
 - Agent personal mailbox sending requires delegated auth flow and explicit mailbox-permission governance.
...
2. **Add a "Send as Email" toggle** to the reply composer (from Phase 2, Task 2.6):
 ...
 - When ON, send from the ticket's company shared mailbox and display it as read-only sender info
...
- [ ] Email replies are sent from the configured company shared mailbox in v1
